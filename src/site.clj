...
(defn- expand-images
  "Parses the hacky, `key1=val1, key2=val2, ...` syntax stuffed into the alt text field of the images in the
  garden_posts/ Markdown.

  Currently supported keys:
  - id     --> becomes an id on the <div class=\"img-container\">...</div>, wrapping the <img> element
  - class  --> appends one or more classes to the \"img-container\" class string
...
(defn- open-external-links-in-new-tabs [form]
  (if (and ; <a> tag
           (vector? form) (= :a (first form))
           ; with an external link
           (not (u/internal-href? (:href (second form)))))
    (assoc-in form [1 :target] "_blank")
    form))

(defn- post-process-generated-hiccup
  "Takes the hiccup generated by the 'parse' step and applies some last-minute transformations. For instance, ensuring
...
(defn render!
  ...
  in `assets/html/` (whose path is returned on success). By default, `:safe` is `true` and instead, the generated
  HTML is pretty-printed."
  ...
  (let [page           (when (keyword? page-kw-or-post-filename) page-kw-or-post-filename)
        ...
        hiccup         (cond
                         ; A. We're dealing with a top-level index page
                         page
                         (-> ((page->hiccup-fn page))
                             (wrap-index-page-boilerplate page))

                         ; B. We're dealing with a garden_posts/ post
                         full-post-path
                         (cond-> (slurp full-post-path)
    ...
    (if safe?
      ; safe? --> pretty-print the generated html
      (u/pphtml html)

      ; not safe? --> write html to a file
